...
## 1.6. Boxplots for DE
### 1.6.1. DE for liquid only, vs C0L
```{r}
# Per strain
ggplot(data = resp, aes(x = Strain.Type, y = nDEP, group = Strain.Type, fill = Strain.Type)) +
  geom_boxplot() +
...
# Per passage
ggplot(data = resp, aes(x = Passage, y = nDEP, group = Passage, fill = Passage)) +
  geom_boxplot() +
  geom_jitter(color="black", size=2, alpha=0.9) +
  theme_light() +
  xlab("Passage") +
...
### 1.6.2. DE vs. C0L - OLD, PROBABLY DELETE
```{r}
# Per strain
...
### 1.6.3. DE vs. C0P - OLD, PROBABLY DELETE
```{r}
# Per strain
...
## 2.2. Classify strains into high, medium or low tolerance and resistance based on these plots, in order to interpret proteomics results based on this. We add this as a new column to the metadata. Current thresholds for classification come from Wenxi but we are not confident on them at all, will show this in the microbiology meeting and see what people think about them. 
```{r}
...
# By resistance and tolerance together
create_heatmap(protein_mat = protein_mat,
             metadata = metadata,
             annotation = "single",
             heatmap_type = "z-score",
             z_score_type = "standard",
             color_palette = "viridis",
             metadata_column_for_annotation = "Resistance_Tolerance",
             metadata_column_for_sample_name = "Sample.Name",
             location_to_save = paste(location_to_save, "Output/Plots/heatmaps/", sep=""),
             save_as = "png",
             plot_name = "default",
...
### 2.3.2. Separate by treatment groups
```{r}
treatments <- unique(metadata$Treatment)[!(unique(metadata$Treatment) %in% c("QC"))]
