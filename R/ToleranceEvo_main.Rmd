...
## 4.2. Perform GSEA with KEGG annotation - as described by https://biostatsquid.com/fgsea-tutorial-gsea/
...
# 5. Repeating DEA and GSEA separately for each treatment group, as suggested during the microbiology meeting
## 5.1. Differential abundance analysis for resistance and tolerance, separately for each treatment group - THE GSEA FOR THE DATA PRODUCED HERE IS IN SECTION 
...
### 5.1.1. Differential abundance analysis
```{r}
...
### 5.1.2. Create a volcano plot for each of the 6 results from differential analysis
```{r}
...
### 5.2.1. Prepare ranked gene lists
For now, my score/rank is: sign(logFC)*(-log10(adj.P.Val))
```{r}
# Since we are going to start an iteration before, we first create an empty list where we can save the output of each of the iterations
